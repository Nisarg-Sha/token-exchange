...
export default function Swap() {
  ...
  const [selectedInputToken, setselectedInputToken] = useState<Token>({
    symbol: "SOL",
    logoURI:
      "https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/So11111111111111111111111111111111111111112/logo.png",
  });
  const [selectedOutputToken, setselectedOutputToken] = useState<Token>({
    symbol: "USDC",
    logoURI:
    ...
    async function getPrice() {
      try {
        const res = await fetch(
          `https://price.jup.ag/v6/price?ids=${selectedInputToken.symbol}&vsToken=${selectedOutputToken.symbol}`
        );
        const data = await res.json();
        const price =
          data.data[selectedInputToken.symbol].price * inputAmount || 0;
      ...
      if (confirmation) {
        toast({
          ...
          action: (
            <Button>
              <Link
                href={`https://explorer.solana.com/tx/${sign}?cluster=mainnet-beta`}
              >
                Show In Explorer
              </Link>
            </Button>
  ...
  return (
    <div className="flex flex-col">
      <h1 className="mx-auto mb-5 justify-center opacity-80 text-[3.5rem] font-bold z-20 bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-400">
        Swaps
      </h1>
      <div className="relative rounded-lg w-[32rem] mt--64 overscroll-y-none">
        ...
        <div className="relative bg-black bg-opacity-[0.85] z-10 p-6 rounded-lg w-[32rem] text-white items-center justify-center">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex space-x-2">
              <div className="">
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      variant="outline"
                      className="rounded-full h-6 w-[5rem] mb-8"
                    >
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter className="flex">
                      <Input
                        ...
                        className="col-span-2 w-full bg-black text-white h-10"
                        onChange={(e) => {
                    ...
                    {slippage > 5 && (
                      <Alert color="yellow" variant="destructive">
                    ...
                    {slippage > 50 && (
                      <Alert color="yellow" variant="destructive">
                        <AlertCircle className="h-4 w-4" />
                        <AlertDescription>
                          Please set a slippage value that is within 0% to 50%
                        </AlertDescription>
                      </Alert>
                    )}
                  </AlertDialogContent>
              ...
              <Button
                ...
                className="bg-[#333333] w-10 h-6 text-[0.5rem] px-2 font-bold text-slate-400"
              >
                MAX
              </Button>
              <Button
          ...
          <div className="border-white border-[1px] p-4 rounded-lg flex items-center justify-between mb-4">
            ...
            />
            <h3 className="text-1xl font-bold ml-8">
              {selectedInputToken.symbol}
            </h3>
            <div className="ml-[7rem] fixed">
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  className="h-36 w-52 overflow-y-scroll"
                  align="end"
                >
                  {topTokens.map((token: Token) => {
                    return (
                      <DropdownMenuItem
                        ...
                        className={
                          selectedInputToken.symbol === token.symbol
                            ? "bg-gray-100 dark:bg-gray-800 py-1 cursor-pointer w-52"
                            : "py-1 cursor-pointer w-52"
                        }
                      >
            ...
            <div className="text-right">
              <Input
                ...
                id="inputamount"
                autoComplete="off"
                className="text-white w-64 text-1.5xl h-full font-semibold border-transparent border-none outline-0 shadow-none text-right bg-transparent"
                placeholder="0.00"
                value={inputAmount}
                required
          ...
          >
            <g stroke="#1c274c" strokeWidth="1.5">
              ...
              <g strokeLinecap="round" strokeLinejoin="round">
                <path d="m9.5 8v8m0 0-2.5-2.75m2.5 2.75 2.5-2.75" />
                <path d="m14.5 16v-8m0 0-2.5 2.75m2.5-2.75 2.5 2.75" />
              </g>
            </g>
          </svg>
          <div className="flex justify-between items-center mb-4">
                  ...
                  {topTokens.map((token: Token) => {
                    return (
                      <DropdownMenuItem
                        ...
                        className={
                          selectedOutputToken.symbol === token.symbol
                            ? "bg-gray-100 dark:bg-gray-800 py-1 cursor-pointer w-52"
                            : "py-1 cursor-pointer w-52"
                        }
                      >
